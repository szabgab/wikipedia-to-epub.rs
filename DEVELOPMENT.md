...
* `{{USS|Missouri|BB-63|6}}`, `{{HMS|Jamaica|44|6}}`, `{{SMS|Nassau|1908}}`, `{{SS|City of Rio de Janeiro}}`, and `{{ship|Japanese cruiser|Kiso}}` render as formatted, italicized ship names linked to their respective articles
...
* `{{glossary}}` and `{{glossary end}}` are silent templates and are omitted
* `{{sronly|text}}` acts as a passthrough formatting template, displaying the visible text: `text`
...
* `{{Literal translation|Eastern Capital}}` (or `{{lit}}` / `{{lit.}}`) displays the literal meaning of a term prefixed by "lit. "
...
* I-series formatting and identifier templates render in simplified form: `{{IAST|karma}}` transliterates Sanskrit text, `{{increase}}` renders an upward marker, `{{indent|3}}` inserts spaces, `{{interp|text}}` brackets interpolated text, `{{INRConvert|5|c}}` renders a rupee amount, `{{ISBNT|9780000000000}}` renders like `{{ISBN}}`, and `{{IPAblink|a}}`, `{{IPAlink|a}}`, `{{IPA link|a}}`, `{{IPAc-cmn|...}}`, and `{{IPAc-hu|...}}` render IPA spans
* I-series external-link templates such as `{{IBDB name}}`, `{{IMDb event}}`, `{{IMO results}}`, `{{IMSLP}}`, `{{INSEE}}`, `{{Instagram}}`, `{{In Our Time}}`, `{{Internet Archive}}`, `{{Internet Archive author}}`, `{{Internet Archive film}}`, and `{{ISU short track skater}}` render concise external links
* I-series country and sports templates such as `{{IDN}}`, `{{INA}}`, `{{IND}}`, `{{IRI}}`, `{{IRL}}`, `{{IRN}}`, `{{IRQ}}`, `{{ISL}}`, `{{ISR}}`, `{{ITA}}`, and `{{ih}}` render simplified article links
