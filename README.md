...
* Observed Wikipedia navigation templates such as `{{History of Korea}}`, `{{Korea topics}}`, `{{East Asian topics}}`, `{{Portal bar}}`, `{{Portal}}`, `{{Commons category}}`, `{{Seoul}}`, `{{Navboxes}}`, and `{{Authority control}}` are omitted
...
* Maintenance and metadata templates such as `{{unreferenced section}}`, `{{Excessive citations inline}}`, `{{location map+}}`, and `{{DEFAULTSORT:...}}` are omitted
