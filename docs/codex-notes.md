...
* `src/main.rs`
  ...
  * Added silent skipping for templates such as `Redirect`, `pp-semi-indef`, `Sfn`, `efn`, `Refbegin`, `Refend`, and succession templates prefixed with `s-`.
...
* `src/tests.rs`
  * Added unit coverage for `Cite report`, `harvc`, `As of`, `Blockquote`, `Further`, `Refbegin`/`Refend`, and silent `s-` template handling.
