...
* `src/main.rs`
  * Added or extended rendering for templates including `ill`, `Reign`, `lang`, `langx`, `Percentage`, `UN Population`, `Korean/auto`, `Ko-translit`, `Cite report`, and `harvc`.
...
* `src/tests.rs`
  * Added unit coverage for `Cite report` and `harvc` rendering.
