...
* `src/main.rs`
  * Added or extended rendering for templates including `ill`, `Reign`, `lang`, `langx`, `Percentage`, `UN Population`, `Korean/auto`, `Ko-translit`, and `Cite report`.
...
* `expected/korea/OEBPS/chapter-3.xhtml`
  * Updated the Sejong bibliography fixture after `Cite report` rendering changed the generated EPUB output.
* `src/tests.rs`
  * Added unit coverage for `Cite report` rendering.
